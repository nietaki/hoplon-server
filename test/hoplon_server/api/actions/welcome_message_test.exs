defmodule HoplonServer.API.Actions.WelcomeMessageTest do
  use ExUnit.Case

  alias HoplonServer.API.Actions.WelcomeMessage

  test "returns welcome message for a name" do
    request =
      Raxx.request(:POST, "/")
      |> HoplonServer.API.set_json_payload(%{name: "Fiona"})
  ...
  test "returns bad request for bad payload" do
    request =
      Raxx.request(:POST, "/")
      |> HoplonServer.API.set_json_payload(%{})
